...
public class OrderUtil {
    ...
    public static String orderStatusText(JnOrder order) {
    ...
    public static OrderHandleOption build(JnOrder order) {
        int status = order.getOrderStatus().intValue();
    ...
    public static boolean isCreateStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_CREATE == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isPayStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_PAY == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isShipStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_SHIP == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isConfirmStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_CONFIRM == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isCancelStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_CANCEL == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isAutoCancelStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_AUTO_CANCEL == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isRefundStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_REFUND == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isRefundConfirmStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_REFUND_CONFIRM == jnOrder.getOrderStatus().shortValue();
    }

    public static boolean isAutoConfirmStatus(JnOrder jnOrder) {
        return OrderUtil.STATUS_AUTO_CONFIRM == jnOrder.getOrderStatus().shortValue();
    }
    ...
    public static String orderHtmlText(JnOrder order, String orderIds, List<JnOrderBooks> orderBooksList) {
        String orderStr = "";
        if (orderBooksList == null || orderBooksList.size() < 1) {// 无书籍时，设置书籍信息为null
            orderStr = null;
        } else {
            for (JnOrderBooks dog : orderBooksList) {
                orderStr = orderStr
                        + "<br>&nbsp;&nbsp;&nbsp;&nbsp;书籍序列码" + "【" + dog.getBooksSn() + "】;"
                        + "<br>&nbsp;&nbsp;&nbsp;&nbsp;书籍名称" + "【" + dog.getBooksName() + "】;"
                        + "<br>&nbsp;&nbsp;&nbsp;&nbsp;书籍规格" + "【" + JSONObject.toJSONString(dog.getSpecifications()) + "】;"
                        + "<br>&nbsp;&nbsp;&nbsp;&nbsp;书籍规格数量" + "【" + dog.getNumber() + "】<br>";
            }
        }
        ...
        if (orderStr != null) {
            sb.append("<br>订购书籍：").append(orderStr);
        }
