<template>
  <div class="app-container">
    ...
    <el-card class="box-card">
      <h3>书籍介绍</h3>
      <el-form ref="books" :rules="rules" :model="books" label-width="150px">
        <el-form-item label="书籍编号" prop="booksSn">
          <el-input v-model="books.booksSn"/>
        </el-form-item>
        <el-form-item label="书籍名称" prop="name">
          <el-input v-model="books.name"/>
        </el-form-item>
        <el-form-item label="当前价格" prop="retailPrice">
          <el-input v-model="books.retailPrice" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="是否新品" prop="isNew">
          <el-radio-group v-model="books.isNew">
            <el-radio :label="true">新品</el-radio>
            <el-radio :label="false">非新品</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="是否热卖" prop="isHot">
          <el-radio-group v-model="books.isHot">
            <el-radio :label="false">普通</el-radio>
            <el-radio :label="true">热卖</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="是否在售" prop="isOnSale">
          <el-radio-group v-model="books.isOnSale">
        ...
        <el-form-item label="书籍图片">
          <el-upload
            ...
            <img v-if="books.picUrl" :src="books.picUrl" class="avatar">
        ...
        <el-form-item label="书籍单位">
          <el-input v-model="books.unit" placeholder="本 / 册 / 套"/>
        </el-form-item>
        ...
        <el-form-item label="书籍简介">
          <el-input v-model="books.brief"/>
        </el-form-item>

        <el-form-item label="书籍详细介绍">
          <editor :init="editorInit" v-model="books.detail"/>
        </el-form-item>
      </el-form>
    </el-card>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="attributes">
        <el-table-column property="attribute" label="书籍参数名称"/>
        <el-table-column property="value" label="书籍参数值"/>
        <el-table-column align="center" label="操作" width="100" class-name="small-padding fixed-width">
          <template slot-scope="scope">
      ...
      <el-dialog :visible.sync="attributeVisiable" title="设置书籍参数">
        <el-form ref="attributeForm" :model="attributeForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
          <el-form-item label="书籍参数名称" prop="attribute">
            <el-input v-model="attributeForm.attribute"/>
          </el-form-item>
          <el-form-item label="书籍参数值" prop="value">
    ...
    <div class="op-container">
      ...
      <el-button type="primary" @click="handleEdit">更新书籍</el-button>
    </div>
...
<script>
import { detailbooks, editbooks, listCatAndBrand } from '@/api/books'
...
export default {
  ...
  methods: {
    init: function() {
      ...
      const booksId = this.$route.query.id
      detailbooks(booksId).then(response => {
        ...
        for (var i = 0; i < this.books.gallery.length; i++) {
          this.galleryFileList.push({
            url: this.books.gallery[i]
          })
        }
        const keywords = response.data.data.books.keywords
        if (keywords !== null) {
    ...
    handleCategoryChange(value) {
      this.books.categoryId = value[value.length - 1]
    },
    ...
    handleClose(tag) {
      ...
      this.books.keywords = this.keywords.toString()
    },
    showInput() {
    ...
    uploadPicUrl: function(response) {
      this.books.picUrl = response.data.url
    },
    ...
    handleGalleryUrl(response, file, fileList) {
      if (response.errno === 0) {
        this.books.gallery.push(response.data.url)
      }
    },
    handleRemove: function(file, fileList) {
      for (var i = 0; i < this.books.gallery.length; i++) {
        ...
        if (this.books.gallery[i] === url) {
          this.books.gallery.splice(i, 1)
        }
      }
    },
