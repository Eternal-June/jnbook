  ...
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="书籍序号或ID">
          <el-tag v-for="tag in dataForm.books" :key="tag" closable type="primary" @close="handleClose(tag)">
            {{ tag }}
          </el-tag>
          <el-input
            v-if="newBooksVisible"
            ref="newBooksInput"
            v-model="newGoodIdSn"
            class="input-new-books"
            size="small"
        ...
        <el-form-item label="书籍低价" prop="price">
...
export default {
  ...
  methods: {
    ...
    handleClose(tag) {
      this.dataForm.books.splice(this.dataForm.books.indexOf(tag), 1)
    },
    showInput() {
      this.newBooksVisible = true
      this.$nextTick(_ => {
        this.$refs.newBooksInput.$refs.input.focus()
      })
    },
    handleInputConfirm() {
      const newGoodIdSn = this.newGoodIdSn
      if (newGoodIdSn) {
        this.dataForm.books.push(newGoodIdSn)
      }
      this.newBooksVisible = false
      this.newGoodIdSn = ''
    },
    handleDownload() {
