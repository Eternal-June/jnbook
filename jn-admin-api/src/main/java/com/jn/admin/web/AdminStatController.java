...
import com.jn.admin.util.StatVo;
import com.jn.core.util.ResponseUtil;
import com.jn.db.service.StatService;
...
public class AdminStatController {
    ...
    @RequiresPermissions("admin:stat:books")
    @RequiresPermissionsDesc(menu = {"统计管理", "书籍统计"}, button = "查询")
    @GetMapping("/books")
    public Object statBooks() {
        logger.info("请求开始=>统计管理->书籍统计->查询");

        List<Map> rows = statService.statBooks();
        String[] columns = new String[]{"day", "orders", "products", "amount"};
        StatVo statVo = new StatVo();
        statVo.setColumns(columns);
        statVo.setRows(rows);

        logger.info("请求结束=>统计管理->书籍统计->查询,响应结果:{}", JSONObject.toJSONString(statVo));
        return ResponseUtil.ok(statVo);
    }
