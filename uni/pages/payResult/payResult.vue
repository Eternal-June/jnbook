  ...
  <view class="container">
    <view class="pay-result">
      <view class="success" v-if="status">
        ...
        <view class="btns">
          <navigator class="btn" url="/pages/ucenter/order/order" open-type="redirect"
            >查看订单</navigator
          >
          <navigator class="btn" url="/pages/index/index" open-type="switchTab"
            >继续逛</navigator
          >
        </view>
      </view>
    ...
    payOrder() {
      ...
      uni.showModal({
        ...
        success: function (res) {
          if (res.confirm) {
            util
              .request(api.OrderPrepay, { orderId: that.data.orderId }, "POST")
              .then((res) => {
                if (res.errno === 0) {
                  that.setData({
                    status: true,
