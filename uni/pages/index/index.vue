    ...
    <view class="container">
      ...
      >
        <swiper-item v-for="(item, index) in banner" :key="index">
          <navigator :url="'/pages/books/books?id=' + item.link">
           <view>
                <text class="txt jnbook_f4"></text>
                <img :src="item.url" background-size="cover">
                <span style="vertical-align:top; font-size: 2.1em; color: yellow; position: absolute; bottom: 0.5em; left: 0.5em;">{{item.name}}</span>
            </view>
          </navigator>
        </swiper-item>
      </swiper>
      ...
      <view class="news acea-row row-between-wrapper">
        <view class="ariticle"><text style="color: red; font-size: 1.2em;">系统公告</text></view>
      ...
      <view class="m-menu jnbook_bg0">
        ...
        >
          ...
          <text class="jnbook_f2">{{ item.name }}</text>
        </navigator>
        <navigator
          hover-class="none"
          class="item"
        ...
        >
          <image
            src="/static/static/images/more.png"
            background-size="cover"
          ></image>
          <text class="jnbook_f2">更多分类</text>
        </navigator>
      </view>
        ...
        <view class="h">
          <view>
            <navigator url="/pages/topic/topic">
              <text class="txt jnbook_f4">书单推荐</text>
            </navigator>
          </view>
        </view>
        <view class="b">
          <scroll-view scroll-x class="list">
            <view v-for="(item, index) in topics" :key="index" class="item">
              <navigator :url="'../topicDetail/topicDetail?id=' + item.id">
                <image class="img" :src="item.picUrl" background-size="cover"></image>
                <view class="np">
                  <text class="name jnbook_f7">{{ item.title }}</text>
                  <text class="price jnbook_f8">￥{{ item.price }}元起</text>
                </view>
                <text class="desc jnbook_f5">{{ item.subtitle }}</text>
              </navigator>
            </view>
          </scroll-view>
        </view>
      </view>
      <view class="a-section a-new" v-if="newbooks.length > 0">
        <view class="title acea-row row-between-wrapper">
          <view class="text">
            <view class="name jnbook_f4"
              >新品首发<text class="new font-color">NEW~</text></view
            >
            <view class="desc jnbook_f5">最新上架</view>
          </view>
          <navigator class="more jnbook_f6" hover-class="none" url="../newBooks/newBooks"
            >更多<text class="iconfont icon-gengduo"></text>
          </navigator>
        </view>
        <view class="b">
          <view v-for="(item, index) in newbooks" :key="index" class="item">
            <navigator :url="'../books/books?id=' + item.id">
              <image class="img" :src="item.picUrl" background-size="cover"></image>
      ...
      <view v-for="(item, index) in floorbooks" :key="index" class="book-grid">
        <view class="h">
          <text class="jnbook_f4">{{ item.name }}</text>
        </view>
        <view class="b">
          <block v-for="(iitem, iindex) in item.booksList" :key="iindex">
            <view :class="'item ' + (iindex % 2 == 0 ? '' : 'item-b')">
              <navigator :url="'../books/books?id=' + iitem.id" class="a">
                <image class="img" :src="iitem.picUrl" background-size="cover"></image>
                <text class="name jnbook_f7">{{ iitem.name }}</text>
                <text class="price jnbook_f8">￥{{ iitem.retailPrice }}</text>
              </navigator>
            </view>
          </block>
        </view>
        <navigator :url="'/pages/category/category?id=' + item.id" class="t">
          <view class="txt jnbook_f6">{{ "更多" + item.name + "好物 >" }}</view>
        </navigator>
      </view>
    </view>
  </view>
...
export default {
  ...
  onLoad: function (options) {
    ...
    if (options.scene) {
      //这个scene的值存在则证明首页的开启来源于朋友圈分享的图,同时可以通过获取到的bookId的值跳转导航到对应的详情页
  ...
  methods: {
    getIndexData: function () {
      ...
      util.request(api.IndexUrl).then(function (res) {
        ...
        if (res.errno === 0) {
          that.setData({
            newbooks: res.data.newbooksList,
            hotbooks: res.data.hotbooksList,
            topics: res.data.topicList,
            floorbooks: res.data.floorbooksList,
...
.container .a-section .hot-bg {
  width: 100%;
  height: 215rpx;
  background-image: url("https://jnbook.oss-cn-shanghai.aliyuncs.com/hot.png");
