...
public class StorageService {
    ...
    public String store(InputStream inputStream, long contentLength, String contentType, String fileName) {
        ...
        JnStorage storageInfo = new JnStorage();
        storageInfo.setName(fileName);
        storageInfo.setSize((int) contentLength);
        storageInfo.setType(contentType);
        storageInfo.setKey(key);
        storageInfo.setUrl(url);
        JnStorageService.add(storageInfo);
    ...
    private String generateKey(String originalFilename) {
        ...
        String key = null;
        JnStorage storageInfo = null;

        do {
            key = CharUtil.getRandomString(20) + suffix;
            storageInfo = JnStorageService.findByKey(key);
